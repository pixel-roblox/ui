...
function pixel_Lib.RippleEffect(btnInstance, color)
...
function pixel_Lib:ToggleUI()
    ...
    if pixel_Lib.Settings.TogglingUI then
        return
    end

    pixel_Lib.Settings.TogglingUI = true
    ...
    if self.Open then
        ...
        pixel_Lib.Settings.TogglingUI = false
    else
...
function pixel_Lib:Notify(props)
    props = props or {}
...
function pixel_Lib:_internal_createloader(options)
    ...
    self.LoaderLogo.Image = "rbxassetid://108069710329003"
...
function pixel_Lib:_internal_createmainwindow(options)
    ...
    self.ScriptHubName.Position = UDim2.new(0, 20, 0.78, -9)
    self.ScriptHubName.Size = UDim2.new(1, -40, 0, 18)
...
function pixel_Lib:InitTabs(tabNames)
    self.Tabs = {}
    self.CurrentTab = nil
...
function pixel_Lib:SwitchTab(tabName)
    if self.CurrentTab then
        pixel_Lib.Tween(self.CurrentTab.TabButton, {BackgroundColor3 = pixel_Lib.Theme.Tab})
        self.CurrentTab.Content.Visible = false
    end
    
    self.CurrentTab = self.Tabs[tabName]
    pixel_Lib.Tween(self.CurrentTab.TabButton, {BackgroundColor3 = pixel_Lib.Theme.TabAccent})
    self.CurrentTab.Content.Visible = true
end

function pixel_Lib:_createButton(tabName, props, flag)
    props = props or {}
    ...
    btnInstance.TextColor3 = pixel_Lib.Theme.Text
    btnInstance.TextSize = 16
    btnInstance.BackgroundColor3 = pixel_Lib.Theme.Tab
    ...
    btnInstance.MouseEnter:Connect(function()
        pixel_Lib.Tween(btnInstance, {BackgroundColor3 = pixel_Lib.Theme.Accent})
    end)
    
    btnInstance.MouseLeave:Connect(function()
        pixel_Lib.Tween(btnInstance, {BackgroundColor3 = pixel_Lib.Theme.Tab})
    end)
    
    btnInstance.MouseButton1Click:Connect(function()
        pixel_Lib.RippleEffect(btnInstance)
...
function pixel_Lib:_createLabel(tabName, props)
    props = props or {}
    local tab = self.Tabs[tabName]
    if not tab then return nil end
...
function pixel_Lib:_createSplitter(tabName, props)
    props = props or {}
    local tab = self.Tabs[tabName]
    if not tab then return nil end
...
function pixel_Lib:_createSlider(tabName, props, flag)
    props = props or {}
    local tab = self.Tabs[tabName]
    if not tab then return nil end
...
function pixel_Lib:_createToggle(tabName, props, flag)
    props = props or {}
    local tab = self.Tabs[tabName]
    if not tab then return nil end
...
function pixel_Lib:_createDropdown(tabName, props, flag)
    props = props or {}
    local tab = self.Tabs[tabName]
    if not tab then return nil end
    ...
    selectionButton.TextColor3 = pixel_Lib.Theme.Text
    selectionButton.TextSize = 14
    selectionButton.TextXAlignment = Enum.TextXAlignment.Left
    selectionButton.BackgroundColor3 = pixel_Lib.Theme.Tab
    ...
    optionsFrame.BackgroundColor3 = pixel_Lib.Theme.Content
    ...
    optionsList.ScrollBarImageColor3 = pixel_Lib.Theme.Accent
    ...
    local function createOptions()
        ...
        for i, option in next, options do
            ...
            optionButton.TextColor3 = pixel_Lib.Theme.Text
            optionButton.TextSize = 14
            optionButton.TextXAlignment = Enum.TextXAlignment.Left
            optionButton.BackgroundColor3 = pixel_Lib.Theme.Tab
            ...
            if option == selectedOption then
                optionButton.BackgroundColor3 = pixel_Lib.Theme.Accent
            end
            
            optionButton.MouseEnter:Connect(function()
                if option ~= selectedOption then
                    pixel_Lib.Tween(optionButton, {BackgroundTransparency = 0.2})
                end
            end)
            
            optionButton.MouseLeave:Connect(function()
                if option ~= selectedOption then
                    pixel_Lib.Tween(optionButton, {BackgroundTransparency = 0.5})
                end
            end)
                ...
                for _, btn in pairs(optionsList:GetChildren()) do
                    if btn:IsA("TextButton") then
                        if btn.Name == selectedOption then
                            pixel_Lib.Tween(btn, {BackgroundColor3 = pixel_Lib.Theme.Accent, BackgroundTransparency = 0})
                        else
                            pixel_Lib.Tween(btn, {BackgroundColor3 = pixel_Lib.Theme.Tab, BackgroundTransparency = 0.5})
                        end
                    end
                end
    ...
    local function toggleDropdown(forceState)
        ...
        if isOpen then
            ...
            pixel_Lib.Tween(optionsFrame, {Size = UDim2.new(0, absSize.X, 0, targetHeight)}, 0.2)
            pixel_Lib.Tween(arrowIcon, {Rotation = 180}, 0.2)
            
            createOptions()
        else
            pixel_Lib.Tween(optionsFrame, {Size = UDim2.new(0, optionsFrame.AbsoluteSize.X, 0, 0)}, 0.2)
            pixel_Lib.Tween(arrowIcon, {Rotation = 0}, 0.2)
...
function pixel_Lib:_createKeybind(tabName, props, flag)
    props = props or {}
    local tab = self.Tabs[tabName]
    if not tab then return nil end
    ...
    keyButton.TextColor3 = pixel_Lib.Theme.Text
    keyButton.TextSize = 14
    keyButton.BackgroundColor3 = pixel_Lib.Theme.Tab
    ...
    local function updateVisuals()
        if listening then
            keyButton.Text = "..."
            pixel_Lib.Tween(keyButton, {BackgroundColor3 = pixel_Lib.Theme.Accent})
        else
            keyButton.Text = keys[selectedKey] or (type(selectedKey) == "EnumItem" and selectedKey.Name) or "None"
            pixel_Lib.Tween(keyButton, {BackgroundColor3 = pixel_Lib.Theme.Tab})
        end
    end
    
    keyButton.MouseEnter:Connect(function()
        if not listening then
            pixel_Lib.Tween(keyButton, {BackgroundColor3 = pixel_Lib.Theme.TabAccent})
        end
    end)
...
function pixel_Lib:_createTextField(tabName, props, flag)
    props = props or {}
    local tab = self.Tabs[tabName]
    if not tab then return nil end
    ...
    textBox.TextColor3 = pixel_Lib.Theme.Text
    textBox.PlaceholderColor3 = pixel_Lib.Theme.SubText
    textBox.TextSize = 14
    textBox.TextXAlignment = Enum.TextXAlignment.Left
    textBox.TextWrapped = false
    textBox.ClearTextOnFocus = props.ClearTextOnFocus ~= nil and props.ClearTextOnFocus or false
    textBox.BackgroundColor3 = pixel_Lib.Theme.Tab
    ...
    if props.CharLimit then
        ...
        charLimit.TextColor3 = pixel_Lib.Theme.SubText
    ...
    textBox.Focused:Connect(function()
        pixel_Lib.Tween(textBox, {BackgroundColor3 = pixel_Lib.Theme.TabAccent})
    end)
    
    textBox.FocusLost:Connect(function(enterPressed)
        pixel_Lib.Tween(textBox, {BackgroundColor3 = pixel_Lib.Theme.Tab})
